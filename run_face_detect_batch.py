...
import math
import random
caffe_root = '/home/tony_jun/caffe'
...
def face_detection(imgFile):
    net_full_conv = caffe.Net('/home/tony_jun/facedetect/deploy_full_conv.prototxt',
                              '/home/tony_jun/facedetect/alexnet_iter_50000_full_conv.caffemodel',
                              caffe.TEST)
    ...
    for scale in scales:
        ...
        cv2.imwrite('/home/tony_jun/facedetect/preprocess/scale_img.jpg',scale_img)
        #scale_img.save("tmp{0}.jpg".format(randNum))
        # load input and configure preprocessing
        #im = caffe.io.load_image("tmp{0}.jpg".format(randNum))
        im = caffe.io.load_image('/home/tony_jun/facedetect/preprocess/scale_img.jpg')
    ...
    print total_boxes
    boxes_nms = np.array(total_boxes)
    true_boxes = nms_average(boxes_nms, 1, 0.5)
    if not true_boxes == []:
        num_detected=true_boxes.__sizeof__()
        print('detect num = ')
        print(num_detected)
        for true_box in true_boxes:
            (x1, y1, x2, y2) = true_box[:-1]
            cv2.rectangle(img, (int(x1),int(y1)), (int(x2),int(y2)), (0,255,0))
            win = cv2.namedWindow('test win', flags=0)

            cv2.imshow('test win', img)
            cv2.imwrite('/home/tony_jun/facedetect/preprocess/result.jpg',img)
...
if __name__ == "__main__":
    imgFile = '/home/tony_jun/facedetect/test_multiface.jpg'
